...
class GateSettingsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach (config('hero.models.role')::where('is_special', 0)->pluck('code') as $roleCode) {
            array_push($gateSettings, [
                'role_code' => $roleCode,
                'permission_ids' => config('hero.models.permission')::pluck('id'),
            ]);
        }
        ...
        foreach ($gateSettings as $gateSetting) {
            ...
            if (isset($gateSetting['role_code'])) {
                $role = config('hero.models.role')::where('code', $gateSetting['role_code'])->firstOrFail();
                $gateSetting['role_id'] = $role->id;
                unset($gateSetting['role_code']);
            }

            if (@$gateSetting['email']) {
                $user = config('hero.models.user')::where('email', @$gateSetting['email'])->firstOrFail();
                $gateSetting['user_id'] = $user->id;
                unset($gateSetting['email']);
            }

            $gateSettings = config('hero.models.gate_setting')::create($gateSetting);
